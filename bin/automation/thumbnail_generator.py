...
class ThumbnailGenerator:
    ...
    def create_thumbnail(self,
                        ...
                        video_path: str | None,
                        frame: int,
                        tad:ThumbnailAutomationData, # Thumbnail Automation Data
                        title:str,
        ...
        self.__render_images(tad.image_tad_instances,THUMBNAIL)
        
        text_surf: Surface = self.__get_text(
            font,
            tad.font_size,
            str(episode_number),tad.text_outline,
            tad.color
            )
        
        text_surf: Surface = rotate(text_surf,tad.text_rot)
        
        textX = int(tad.text_pos[0] - (text_surf.get_width() *.5))
        
        textY = int(tad.text_pos[1] - (text_surf.get_height() *.5))
        
        THUMBNAIL.blit(text_surf,(textX,textY))
        
        DM.create_folder(self.__get_path(title))
    ...
    def __render_background(self,
                          tad: ThumbnailAutomationData, #ThumbnailAutomationData
                          video_path: str,
                          frame: float | int,
                          thumbnail: Surface):
        """
